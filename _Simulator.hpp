...
class Simulation {
    ...
    using StaticFieldMatrix = typename std::array<std::array<TypeInMatrix, Columns + 1>, Rows>;
    ...
    template<class TypeInMatrix>
    using StaticMatrix = std::array<std::array<TypeInMatrix, Columns + 1>, Rows>;
    ...
    requires(UseStaticSize) {
        ...
        assert(content.field.front().size() == Columns + 1);

        for (size_t i = 0; auto& row : field) {
            const std::vector<char>& dynamic_field = content.field[i];
...
void RecursiveSimulationStart(const FieldContent& content) {
    ...
    if (StaticSize.rows == content.field.size() && StaticSize.columns == content.field.front().size() - 1) {
...
private:
    ...
    static bool CheckForFixed(std::string_view type_name, std::string_view prefix,
        ...
        std::cout << "Parsing " << type_name << std::endl;
        ...
        std::cout << "Got " << n_str << " and " << k_str << std::endl;
